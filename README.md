...
The **Character Creation** workflow is the 2.0 companion to those reference pages. It turns the structured Core Rulebook contract into a ten-screen, mechanically validated priority builder without copying rule values into page components. The nine mechanical creation stages are followed by a Biography screen for alias and personal details before final review. New drafts begin with no creation level, priorities, metatype, path, Attribute allocation or completed steps. Priority assignment reuses the responsive Priority Array dossier with selectable cells and cards. Gear opens each department at its subcategories, then drills into searchable product cards; catalogue and cart occupy separate toggleable views. Ratings are chosen before a product enters the cart, names open their dedicated archive record in a new tab, and compatible installed attachments or enhancements are configured on the cart line. Ammunition, spare clips, holsters and other standalone weapon support remain independent Weapons catalogue products. Core programs share the same purchasing workflow. Starting Lifestyle is purchased once in Gear and then displayed automatically in Biography and final review. Unspent nuyen carryover is derived from the cart rather than entered separately. Each screen must be mechanically clear and explicitly confirmed before the next unlocks; sidebar, mobile-picker and direct-URL navigation all use that same central progression rule. Attribute point budgets, metatype ceilings, the single-natural-maximum rule and Special Attribute caps are enforced directly by their controls with inline explanations. Runner Concept is not a required screen or draft field. One versioned `CharacterDraft` holds the complete mechanical build; catalogue adapters normalize existing metatype, Quality, skill, spell, ritual, adept-power, equipment, weapon, software, cyberdeck, vehicle, drone and Lifestyle records at the boundary.
