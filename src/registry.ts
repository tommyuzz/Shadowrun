...
const actionRole = (record: ReferenceRecord): string => {
  ...
  if (restriction.includes("counts as") && restriction.includes("attack")) return "Attack action";
  if (restriction.startsWith("not an attack")) return "Non-attack";
  return "Conditional";
};
...
const filterDefinitions: Record<string, FilterDefinition[]> = {
  ...
  attributes: [fieldFilter("linked-skill", "Linked skill", "All linked skills", "common_linked_skills")],
  ...
  actions: [{ id: "attack-role", label: "Attack relationship", allLabel: "All action roles", values: (record) => [actionRole(record)] }],
...
const moduleRows: Omit<ModuleDefinition, "filters">[] = [
  ...
  { id: "actions", name: "Actions", singular: "Action", sector: "corerules", kicker: "Combat timing archive", subtitle: "Fifth edition generic action index", archiveCode: "ACTION // 5E", moduleCode: "Tempo // Action economy", intro: "Generic Free, Simple and Complex Actions, their tests, requirements, attack limits and reload methods.", listInstruction: "Select an action to open its complete timing record", listMeta: actionRole, defaultCategoryId: "free-actions" },
  ...
  { id: "weapons", name: "Weapons", singular: "Weapon", sector: "equipment", kicker: "Street armament archive", subtitle: "Fifth edition weapon and support index", archiveCode: "ARSENAL // 5E", moduleCode: "Arsenal // Weapon index", intro: "Melee, projectile, firearm, heavy and explosive weapons with compatible ammunition and accessories.", listInstruction: "Select a weapon or support item to open its full record", listMeta: (r) => r.subcategory || "Uncategorised" },
