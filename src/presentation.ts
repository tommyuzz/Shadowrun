...
export const presentations: Record<string, PagePresentation> = {
  ...
  lifestyles: {
    ...
    searchLabel: "Search all lifestyle fields", searchPlaceholder: "Enter lifestyle, benefit, cost, rating or rule…", listTitle: (c) => c.label, listEyebrow: (c) => archiveEyebrow(c, "Lifestyle"), recordEyebrow: (r) => r.category === "Lifestyles" ? `${r.subcategory || "Residential"} // Lifestyle profile` : `${r.category} // ${r.subcategory || "Lifestyle"} record`,
    indexPrefix: (c) => c.id === "entertainment" ? "EX" : c.id === "lifestyle-options" ? "LO" : "LS", tagButtonClass: "tag-toggle", tagDetailClass: "tag-detail", tagDetailTitleClass: "tag-detail-title", tagDetailCopyClass: "tag-detail-copy", backLabel: "Back to lifestyle index", tabLabel: (c) => c.label
  },
  cyberdecks: {
