...
const legends: Record<string, [string, string][]> = {
  ...
  actions: [["FREE", "Normally one Free Action per Initiative Pass"], ["SIMPLE", "Normally two Simple Actions per Action Phase"], ["COMPLEX", "Normally one Complex Action per Action Phase"], ["TEST", "Dice pool, limit and threshold or opposition"], ["REQ", "Conditions that must be satisfied"], ["ATK", "How the action interacts with the phase's attack limit"]],
...
export function ModuleSidebar({ module, category, data }: { module: ModuleDefinition; category: ReferenceCategory; data: ReferenceData }) {
  ...
  if (module.id === "weapons") return <aside className="sidebar" aria-label="Weapon archive information"><Panel title={category.label === "Weapon Support" ? "Support protocol" : "Archive scope"}><p className="protocol-copy">{category.label === "Weapon Support" ? "Ammunition and accessories now live beside the weapons they support. Compatibility lists are generated from centralized type, feed and mount profiles so a new matching weapon can participate automatically." : "Melee, projectile, firearm, heavy and explosive weapons share one archive with their compatible ammunition and accessories. Open a weapon record to inspect its available support."}</p></Panel><Legend title={category.label === "Weapon Support" ? "Support field key" : "Weapon field key"} rows={category.label === "Weapon Support" ? [["¥", <><strong>Cost</strong> is the listed price for the support item or ammunition quantity.</>], ["MNT", <><strong>Mount</strong> is the accessory position used on the weapon.</>], ["DV", <><strong>Damage modifier</strong> changes the weapon&apos;s base Damage Value.</>], ["AP", <><strong>AP modifier</strong> changes Armor Penetration.</>], ["FIT", <><strong>Applicable weapons</strong> are inferred from the centralized compatibility profile.</>]] : [["ACC", <><strong>Accuracy</strong> is the weapon&apos;s inherent limit for attack tests, before applicable modifications.</>], ["DV", <><strong>Damage Value</strong> is the weapon&apos;s base damage before net hits and other modifiers.</>], ["AP", <><strong>Armor Penetration</strong> modifies the Armor rating used for the Damage Resistance Test.</>], ["RC", <><strong>Recoil Compensation</strong> offsets recoil from firing multiple rounds. A value in parentheses is conditional.</>], ["MODE", <><strong>Firing Mode</strong>: SS is Single-Shot, SA is Semi-Automatic, BF is Burst Fire, and FA is Full Auto.</>]]}/></aside>;
  ...
  switch (module.id) {
    ...
    case "actions": { const rules = payloadObject(data, "action_rules"); title = `${category.label} protocol`; copy = `${String(rules[category.label] || copy)}<br><br>${String(rules.Scope || "")}`; glitch = String(rules["Unlisted Actions"] || ""); legendTitle = "Action key"; break; }
    ...
    case "equipment": title = category.id === "all" ? "Equipment market" : category.label; copy = category.id === "all" ? "Browse base equipment by market department. Enhancements are embedded into compatible item records so cost, effect and configuration can be reviewed together." : String(categories[category.label] || copy); legendTitle = "Market key"; break;
  }
...
const footers: Record<string, [string, string, string]> = {
  ...
  actions: [">> DECLARE. RESOLVE. ADVANCE.\nCONTROL THE ACTION PHASE.", "One attack limit. Clear requirements.\nSpend every action deliberately.", "⌁"],
