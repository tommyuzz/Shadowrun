...
function simpleData(payload: Record<string, unknown>, collection: string, categoryMode: "category" | "subcategory" | "single" = "category", includeAll = true): ReferenceData {
  ...
  const records = rawRecords.map(([name, raw]) => {
    ...
    return record(name, raw, category, raw.subcategory ? [text(raw.subcategory)] : []);
  });
  ...
  return { records, categories: categoriesFrom(records, descriptions, includeAll), definitions };
}
...
function spellData(payload: Record<string, unknown>): ReferenceData {
  ...
  return { records, categories, definitions };
}
...
function skillData(payload: Record<string, unknown>): ReferenceData {
  ...
  records.forEach((item) => { descriptions[item.category] = `Skills commonly linked to the ${item.category} attribute.`; });
  return { records, categories: categoriesFrom(records, descriptions, true), definitions: {} };
}
...
function cyberdeckData(payload: Record<string, unknown>): ReferenceData {
  ...
  return { records, categories: categoriesFrom(records, lookupDefinitions(payload.category) as Record<string, string>, false), definitions: lookupDefinitions(payload.subcategories) };
}
